...
["Zeus Additions - Utility", "[WIP] Make Vehicle into VBIED", {
    params ["", "_object"];
    ...
    ["Make Vehicle into VBIED", [
        ["TOOLBOX:YESNO", ["Make Vehicle into VBIED", ""], isNil {_object getVariable QGVAR(detonateJIPID)}],
    ...
    ],
    {
        ...
        if (_makeIntoVBIED) then {
            ...
            if (!isNil {_object getVariable QGVAR(detonateJIPID)}) exitWith {};
            ...
            private _jipID = ["zen_common_execute", [{
                ...
                private _ehID = _object addEventHandler ["Engine", {
                    ...
                    // Apply damage to crew; If they don't die, make sure they do
                    {
                        if (zen_common_aceMedical) then {
                            [_x, ace_medical_playerDamageThreshold, "Body", "explosive", objNull, nil, false] call ace_medical_fnc_addDamageToUnit;
                ...
                _object setVariable [QGVAR(detonateActionID), _ehID];
            }, [_object, _IEDSize]]] call CBA_fnc_globalEventJIP;

            _object setVariable [QGVAR(detonateJIPID), _jipID, true];

            [_jipID, _object] call CBA_fnc_removeGlobalEventJIP;

            ["Made vehicle into a VBIED"] call zen_common_fnc_showMessage;
        } else {
            private _jipID = _object getVariable QGVAR(detonateJIPID);
            ...
            ["zen_common_execute", [{
                private _ehID = _this getVariable QGVAR(detonateActionID);

                if (isNil "_ehID") exitWith {};

                _this removeEventHandler ["Engine", _ehID];
                _this setVariable [QGVAR(detonateActionID), nil];
            }, _object]] call CBA_fnc_globalEvent;

            _object setVariable [QGVAR(detonateJIPID), nil, true];
            _object setVariable [QGVAR(IEDSize), nil, true];
