...
[LSTRING(moduleCategoryPlayers), LSTRING(channelVisibilityModuleName), {
    ...
    private _dialogChoices = [
        ...
        ["EDIT", ["str_channel_global", format [LLSTRING(channelVisibilityChangeChannelDesc), localize "str_channel_global"]], [str (channelEnabled 0)], true],
        ["EDIT", ["str_channel_side", format [LLSTRING(channelVisibilityChangeChannelDesc), localize "str_channel_side"]], [str (channelEnabled 1)], true],
        ["EDIT", ["str_channel_command", format [LLSTRING(channelVisibilityChangeChannelDesc), localize "str_channel_command"]], [str (channelEnabled 2)], true],
        ["EDIT", ["str_channel_group", format [LLSTRING(channelVisibilityChangeChannelDesc), localize "str_channel_group"]], [str (channelEnabled 3)], true],
        ["EDIT", ["str_channel_vehicle", format [LLSTRING(channelVisibilityChangeChannelDesc), localize "str_channel_vehicle"]], [str (channelEnabled 4)], true],
        ["EDIT", ["str_channel_direct", format [LLSTRING(channelVisibilityChangeChannelDesc), localize "str_channel_direct"]], [str (channelEnabled 5)], true]
    ];
    ...
    {
        _x params ["", "_name", "", "", "", "_exists"];

        if (_exists) then {
            private _channelID = (_forEachIndex + 1) call BIS_fnc_radioChannelIDToChannelNum;
            _dialogChoices pushBack (["EDIT", [format ["%1: '%2'", localize "str_a3_cfgvehicles_modulechat_f_arguments_channel_0", _name], format [LLSTRING(channelVisibilityChangeChannelDesc), _name]], [str (channelEnabled _channelID)], true]);
            _channelIDs pushBack _channelID;
        };
    } forEach (radioChannelInfo false);
    ...
    [LSTRING(channelVisibilityModuleName), _dialogChoices, {
        ...
        {
            private _channelID = _channelIDs select _forEachIndex;

            if (_x regexMatch "^\[\s*(?:true|false)\s*(?:,\s*(?:true|false)\s*)*\]$") then {
                private _array = parseSimpleArray _x;

                if (count _array == 4 && {_array isEqualTypeAll true}) then {
                    _enableArray pushBack [_channelID, _array];
                } else {
                    _enableArray pushBack [_channelID, channelEnabled _channelID];
                    _warning = true;
                };
            } else {
                _enableArray pushBack [_channelID, channelEnabled _channelID];
        ...
        if (_warning) then {
            systemChat "[Zeus Additions]: One or more arrays had invalid formats and were ignored.";
        };
